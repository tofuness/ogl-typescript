import {R as Renderer, a as Camera, T as Transform, P as Program, M as Mesh, V as Vec3} from "./GLTFSkin.e3c4699d.js";
import {R as RenderTarget} from "./RenderTarget.e49cf8bd.js";
...
`;
{
  let resize = function() {
    ...
    mouse.set(e.x * gl.canvas.width / gl.canvas.clientWidth, gl.canvas.height - e.y * gl.canvas.height / gl.canvas.clientHeight - 1);
  }, update = function(t) {
    ...
    gl.readPixels(mouse.x, mouse.y, 1, 1, gl.RGBA, gl.UNSIGNED_BYTE, data);
    const id = data[0] + (data[1] << 8) + (data[2] << 16) + (data[3] << 24);
    if (id) {
      let data2 = objects[id - 1];
      let [rx, ry, rz] = data2.random;
      highlight.scale.set(0.9 + ry * 0.2).scale(1.005);
      highlight.rotation.set(0);
      highlight.rotation.y = rx * 6.28 + 4 * time * (ry - 0.5);
      highlight.position.set(data2.offset);
  ...
  let mesh, highlight, objects = [];
  {
    ...
    for (let i = 0; i < num; i++) {
      ...
      let id = i + 1;
      idData.set([(id >> 0 & 255) / 255, (id >> 8 & 255) / 255, (id >> 16 & 255) / 255, (id >> 24 & 255) / 255], i * 4);
